...
const hud = await game(() => ({
  ...
  chips: document.querySelectorAll('#turn-order .chip').length,
...
check(hud.hudShown && hud.hp === 3 && hud.chips === 5, `HUD: 3 health bars, 5 turn chips`);
...
await page.click('#btn-hand');
await page.waitForTimeout(150);
check(await game(() => !document.getElementById('hand-overlay').hidden), 'hand panel opens');
